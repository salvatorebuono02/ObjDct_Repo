...
class Client:
    ...
    def __init__(self):
        ...
        self.client_model_io_encoder = None
    ...
    def create_keys(self, profileStr):
        ...
        self.client_context = pyhelayers.HeModel.create_context(client_profile)
        context_buffer = self.client_context.save_to_buffer()
        return context_buffer

    def encrypt_data(self, model_io_encoder_buf, test_img_list):
        """
        Encrypts the data. 
        Load io encoder
        """
        print("Encrypting data on client side.")
        self.client_model_io_encoder = pyhelayers.load_io_encoder(self.client_context, model_io_encoder_buf)
        encrypted_samples = pyhelayers.EncryptedData(self.client_context)
        self.client_model_io_encoder.encode_encrypt(encrypted_samples, [test_img_list])
        encrypted_samples_buf = encrypted_samples.save_to_buffer()
        return encrypted_samples_buf
    
    def decrypt_data(self, enc_predictions_buf):
        """
        Decrypts the data.
        Prameters:
        ...
        client_enc_predictions=pyhelayers.load_encrypted_data(self.client_context,enc_predictions_buf)
        plain_predictions = self.client_model_io_encoder.decrypt_decode_output(client_enc_predictions)
